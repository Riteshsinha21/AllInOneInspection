...
//            } else {
    ...
    fileprivate func _capturePicture(_ imageData: Data, _ imageCompletion: @escaping (UIImage?, NSError?) -> Void) {
        ...
        let cropImage = cropToBounds(image: img, width: 1024, height: 1024)
        let image = fixOrientation(withImage: cropImage)
        
        if writeFilesToPhoneLibrary {
            
            let imagename = "tempImg\(Int(Date().timeIntervalSince1970)).jpg"
            let filePath = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent(imagename)
            let newImageData = _imageDataWithEXIF(forImage:cropImage, imageData) as Data
...
//        }else{
            ...
            return self.imageOrientation(image)
       // }
