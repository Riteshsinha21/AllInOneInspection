...
open class PagerController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate {
	...
	open var selectedTabTextColor = UIColor.yellow
	...
	open var tabLocation: PagerTabLocation! 
	...
	open func selectTabAtIndex(_ index: Int) {
       ...
       //self.selectTabAtIndex(index, swipe: false)
        /*
        if(UserDefaults.standard.bool(forKey: "SaveChangesOrNot"))
        {
            
        
        let alertController = UIAlertController(title: "", message: "Changes that you made may not be saved?", preferredStyle: .alert)
        ...
        let DontSavAction = UIAlertAction(title: "Don't Save", style: UIAlertActionStyle.default) {
            UIAlertAction in
            NSLog("Don't sa Pressed")
            UserDefaults.standard.set(false, forKey: "SaveChangesOrNot")
            self.selectTabAtIndex(index, swipe: false)
        }
        ...
        alertController.addAction(DontSavAction)
	...
	func defaultSettings() {
        ...
        //comment by nilesh for scrolling functionlity remove
	...
	func layoutSubViews() {
		...
		if !ignoreTopBarHeight && self.navigationController?.navigationBar.isTranslucent != false {
			topLayoutGuide = UIApplication.shared.isStatusBarHidden ? 0.0 : 60.0
